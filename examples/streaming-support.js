...
app.use(async (ctx, next) => {
  ...
  console.log(`→ ${ctx.method} ${ctx.url} (Content-Length: ${ctx.headers['content-length'] || 'chunked'})`);
  await next();
...
app.use('/stream', proxy('httpbin.org', {
  ...
  proxyReqOptDecorator(proxyReqOpts, ctx) {
    console.log(`  → Streaming request: ${proxyReqOpts.method} ${proxyReqOpts.path}`);
    return proxyReqOpts;
  },
  
  userResDecorator(proxyRes, proxyResData, ctx) {
    console.log(`  ← Streamed response: ${proxyRes.statusCode} (${proxyResData.length} bytes)`);
    return proxyResData;
  }
}));

// Route 2: Regular mode (parseReqBody: true) - supports retry but not streaming
app.use('/api', proxy('httpbin.org', {
...
app.use(proxy('httpbin.org', {
  ...
  parseReqBody: function(ctx) {
    ...
    const isFileUpload = ctx.path.includes('/upload') || 
    ...
    if (isLargeUpload || isFileUpload) {
      console.log(`  Using streaming mode for ${ctx.path} (${Math.round(contentLength/1024)}KB)`);
...
const port = 3002;
app.listen(port, () => {
  console.log(`Streaming Support Example Server running on http://localhost:${port}`);
  console.log('');
  console.log('Test scenarios:');
  console.log('');
  console.log('🌊 Streaming Mode (parseReqBody: false):');
  console.log(`   curl -X POST -d @package.json http://localhost:${port}/stream/post`);
  console.log(`   curl -X POST -H "Content-Type: text/plain" --data-binary @README.md http://localhost:${port}/stream/post`);
  console.log(`   curl -X POST -T package.json http://localhost:${port}/stream/put`);
  console.log('   ↑ These support large files but retry is disabled for safety');
  console.log('');
  console.log('🔄 Retry Mode (parseReqBody: true):');
  console.log(`   curl -X POST -H "Content-Type: application/json" -d '{"test":"small data"}' http://localhost:${port}/api/post`);
  ...
  console.log(`   curl -X POST -H "Content-Type: application/json" -d '{"small":"data"}' http://localhost:${port}/post`);
  console.log(`   curl -X POST -H "Content-Length: 2000000" -d '$(head -c 2000000 /dev/zero)' http://localhost:${port}/post`);
  console.log(`   curl -X POST -H "Content-Type: multipart/form-data" -F "file=@package.json" http://localhost:${port}/upload`);
  console.log('   ↑ Automatically chooses streaming vs retry based on request size/type');
  console.log('');
  console.log('💡 Key points:');
  console.log('   • parseReqBody: false = Streaming mode (large files OK, no retry)');
  console.log('   • parseReqBody: true = Buffered mode (retry OK, memory limited)');
