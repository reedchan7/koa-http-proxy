...
describe("userResDecorator", function () {
    ...
    app.use(
      proxy("http://localhost", {
        ...
        userResHeadersDecorator: function (headers) {
          var newHeaders = Object.keys(headers).reduce(function (result, key) {
  ...
  it("can change the location of a redirect", function (done) {
    ...
    var redirectingServerOrigin = [
      "http://localhost",
      redirectingServerPort,
    ].join(":");

    var server = redirectingServer(
      redirectingServerPort,
    ...
    proxyApp.use(
      proxy(redirectingServerOrigin, {
        userResDecorator: function (rsp, data, ctx) {
          ...
          ctx.set(
            "location",
            proxyReturnedLocation.replace(redirectingServerPort, preferredPort),
          );
          return data;
        },
      }),
