...
function verifyBuffer(rspd, reject) {
  if (!Buffer.isBuffer(rspd)) {
    return reject(
      new Error("userResDecorator should return string or buffer as data"),
    );
  }
}
...
function updateHeaders(ctx, rspdBefore, rspdAfter, reject) {
  if (!ctx.headerSent) {
    ...
    var error =
      '"Content-Length" is already sent,' +
...
function decorateProxyResBody(container) {
  ...
  var proxyResData = maybeUnzipResponse(
  ...
  return Promise.resolve(resolverFn(proxyRes, proxyResData, ctx)).then(
    function (modifiedResData) {
